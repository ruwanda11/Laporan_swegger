import { useQuery, useQueryClient } from "@tanstack/react-query"; // BARU: Tambah useQueryClient
...
export default function RecipeList({ category, role }: RecipeListProps) {
  ...
  const queryClient = useQueryClient(); // BARU: Inisialisasi queryClient
  ...
  // BARU: Fungsi untuk menangani penghapusan resep
  ...
  const handleDelete = async (id: number) => {
    ...
    if (window.confirm("Yakin ingin menghapus resep lezat ini?")) {
      ...
      try {
        ...
        // Invalidate query agar React Query mengambil data terbaru (otomatis refresh list)
        ...
        alert("Gagal menghapus resep. Pastikan Anda memiliki akses.");
      ...
      {filteredRecipes && filteredRecipes.length > 0 ? (
        ...
        filteredRecipes.map((recipe) => (
          ...
          <div key={recipe.id} className="relative group">
            ...
            {/* BARU: Update Bagian Admin Actions */}
              ...
              <div className="absolute top-6 right-6 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 z-10">
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    e.preventDefault(); // Mencegah klik tombol memicu navigasi ke Detail Recipe
